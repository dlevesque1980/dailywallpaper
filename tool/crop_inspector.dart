...
Future<void> main() async {
  testWidgets('Extract Smart Cropped Images Tool', (WidgetTester tester) async {
    final settings = CropSettings.balanced.copyWith(
      ...
      maxScaleFactor:
          2.0, // This is ignored now internally, but we set it anyway
    ...
    await tester.runAsync(() async {
      for (var filename in files) {
        ...
        if (true) {
          ...
          print(
              'Best Crop: ${best.x}, ${best.y}, ${best.width} x ${best.height}');

          for (final score in result.allScores) {
            print(
                '  [${score.strategy}] score=${score.score.toStringAsFixed(3)} x=${score.coordinates.x.toStringAsFixed(3)} w=${score.coordinates.width.toStringAsFixed(3)}');
          }

          final renderResult = await SmartCropper.processImage(
              filename, image, targetSize, imageSettings);
          final outFilename =
              '$outputDir/cropped_${filename.replaceAll('.jpg', '.png').replaceAll('.jpeg', '.png')}';

          if (renderResult.image != null) {
            final byteData = await renderResult.image!
