  ...
  group('CropCacheDatabase', () {
    ...
    group('CRUD operations', () {
      test('should insert and retrieve cache entry', () async {
        ...
        expect(retrieved.accessCount,
            equals(2)); // Should be incremented on access
      });
        ...
        for (int i = 0; i < entries.length - 1; i++) {
          expect(
            entries[i].lastAccessedAt.isAfter(entries[i + 1].lastAccessedAt) ||
                entries[i]
                    .lastAccessedAt
                    .isAtSameMomentAs(entries[i + 1].lastAccessedAt),
            isTrue,
          );
        }
      });
    ...
    group('statistics and maintenance', () {
      ...
      test('should perform maintenance successfully', () async {
        ...
        expect(result.lruEntriesEvicted,
            equals(1)); // 1 LRU evicted to reach max 2
