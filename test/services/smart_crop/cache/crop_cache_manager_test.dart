...
void main() {
  group('CropCacheManager', () {
    ...
    setUpAll(() async {
      // Initialize FFI for testing (no isolate to avoid parallel DB locking)
      sqfliteFfiInit();
      ...
      // Reset singleton so this test file gets a fresh database
    ...
    group('cache key generation', () {
      test('should generate consistent cache keys', () {
        ...
        final key1 =
            cacheManager.generateCacheKey(imageUrl, targetSize, settings);
        final key2 =
            cacheManager.generateCacheKey(imageUrl, targetSize, settings);
      ...
      test('should generate different keys for different inputs', () {
        ...
        final key1 =
            cacheManager.generateCacheKey(imageUrl1, targetSize, settings);
        final key2 =
            cacheManager.generateCacheKey(imageUrl2, targetSize, settings);
      ...
      test('should generate different keys for different target sizes', () {
        ...
        final key1 =
            cacheManager.generateCacheKey(imageUrl, targetSize1, settings);
        final key2 =
            cacheManager.generateCacheKey(imageUrl, targetSize2, settings);
      ...
      test('should generate different keys for different settings', () {
        ...
        final key1 =
            cacheManager.generateCacheKey(imageUrl, targetSize, settings1);
        final key2 =
            cacheManager.generateCacheKey(imageUrl, targetSize, settings2);
    ...
    group('cache operations', () {
      ...
      test('should invalidate cache when settings change', () async {
        ...
        await cacheManager.cacheCrop(
            imageUrl, targetSize, settings1, coordinates);
        ...
        expect(retrieved,
            isNull); // Should be cache miss due to different settings
      });
      ...
      test('should get cached crops for image URL', () async {
        ...
        for (int i = 0; i < sizes.length; i++) {
          ...
          await cacheManager.cacheCrop(
              imageUrl, sizes[i], settings, coordinates);
        }
      ...
      test('should invalidate image cache', () async {
        ...
        final invalidatedCount =
            await cacheManager.invalidateImageCache(imageUrl);
    ...
    group('cache statistics', () {
      test('should provide accurate statistics', () async {
        ...
        expect(
            stats.averageAccessCount, equals(1.0)); // All entries accessed once
      });
