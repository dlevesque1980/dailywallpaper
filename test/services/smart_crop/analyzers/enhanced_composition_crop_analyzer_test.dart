...
import 'package:dailywallpaper/services/smart_crop/analyzers/enhanced_composition_crop_analyzer.dart';
...
void main() {
  group('EnhancedCompositionCropAnalyzer', () {
    late EnhancedCompositionCropAnalyzer analyzer;

    setUp(() {
      analyzer = EnhancedCompositionCropAnalyzer();
    });
    ...
    group('Analyzer Properties', () {
      test('should have correct name and properties', () {
        expect(analyzer.name, equals('enhanced_composition'));
        expect(analyzer.strategyName, equals('enhanced_composition'));
        expect(analyzer.priority, equals(750));
      ...
      test('should have correct metadata', () {
        ...
        expect(metadata.description, contains('composition'));
        expect(metadata.description, contains('golden ratio'));
    ...
    group('Basic Functionality', () {
      testWidgets('should return valid crop score for simple image',
        ...
        expect(result.strategy, equals('enhanced_composition'));
      ...
      testWidgets('should include composition metrics in results',
          (WidgetTester tester) async {
        final image = _createMockImage(400, 300);
        final targetSize = const ui.Size(200, 150);
        ...
        expect(result.metrics,
            containsPair('rule_of_thirds_score', isA<double>()));
        expect(
            result.metrics, containsPair('golden_ratio_score', isA<double>()));
        expect(
            result.metrics, containsPair('visual_weight_score', isA<double>()));
        expect(result.metrics,
            containsPair('dynamic_symmetry_score', isA<double>()));
        expect(result.metrics, containsPair('balance_score', isA<double>()));
        expect(
            result.metrics, containsPair('composition_score', isA<double>()));
    ...
    group('Composition Rules', () {
      testWidgets('should generate rule of thirds candidates',
          (WidgetTester tester) async {
        final image = _createMockImage(600, 400);
        final targetSize = const ui.Size(300, 200);
        ...
        // Should have reasonable rule of thirds score
        final ruleOfThirdsScore =
            result.metrics['rule_of_thirds_score'] as double? ?? 0.0;
        expect(ruleOfThirdsScore, greaterThanOrEqualTo(0.0));
        expect(ruleOfThirdsScore, lessThanOrEqualTo(1.0));
      });

      testWidgets('should generate golden ratio candidates',
          (WidgetTester tester) async {
        final image = _createMockImage(600, 400);
        final targetSize = const ui.Size(300, 200);
        ...
        // Should have reasonable golden ratio score
        final goldenRatioScore =
            result.metrics['golden_ratio_score'] as double? ?? 0.0;
        expect(goldenRatioScore, greaterThanOrEqualTo(0.0));
        expect(goldenRatioScore, lessThanOrEqualTo(1.0));
      });

      testWidgets('should calculate visual weight distribution',
          (WidgetTester tester) async {
        final image = _createMockImage(400, 300);
        final targetSize = const ui.Size(200, 150);
        ...
        // Should have visual weight score
        final visualWeightScore =
            result.metrics['visual_weight_score'] as double? ?? 0.0;
        expect(visualWeightScore, greaterThanOrEqualTo(0.0));
        expect(visualWeightScore, lessThanOrEqualTo(1.0));
      });
    });
    ...
    group('Error Handling', () {
      ...
      testWidgets(
          'should provide fallback crop when composition analysis fails',
          (WidgetTester tester) async {
    ...
    group('Crop Quality', () {
      testWidgets('should prefer well-composed crops',
          (WidgetTester tester) async {
        final image = _createMockImage(600, 400);
        final targetSize = const ui.Size(400, 300);
        ...
        // Should have reasonable composition score
        final compositionScore =
            result.metrics['composition_score'] as double? ?? 0.0;
        expect(
            compositionScore, greaterThan(0.2)); // Should be better than random
      });

      testWidgets('should avoid extreme edge crops',
          (WidgetTester tester) async {
        final image = _createMockImage(400, 300);
        ...
        // Should have reasonable balance score (avoiding edges)
        final balanceScore = result.metrics['balance_score'] as double? ?? 0.0;
        expect(balanceScore, greaterThan(0.3)); // Should avoid extreme edges
      });
    });
  });
}
