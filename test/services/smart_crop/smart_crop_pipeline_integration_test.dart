...
void main() {
  group('Smart Crop Pipeline Integration Tests', () {
    ...
    group('End-to-End Crop Processing', () {
      test('should process complete workflow from image URL to cropped result',
          () async {
        // Create test image
        ...
        final result = await SmartCropper.processImage(
            imageUrl, sourceImage, targetSize, settings);
        ...
        expect(result.cropResult.processingTime.inMilliseconds,
            greaterThanOrEqualTo(0));
        expect(result.cropResult.fromCache, isFalse); // First time

        // Second processing should use cache
        final cachedResult = await SmartCropper.processImage(
            imageUrl, sourceImage, targetSize, settings);
        expect(cachedResult.success, isTrue);
        expect(cachedResult.cropResult.fromCache, isTrue);
        expect(cachedResult.cropResult.bestCrop,
            equals(result.cropResult.bestCrop));
      });
      ...
      test('should handle different image aspect ratios correctly', () async {
        final testCases = [
          {
            'name': 'Landscape',
            'imageSize': const ui.Size(1600, 900),
            'targetSize': const ui.Size(400, 400)
          },
          {
            'name': 'Portrait',
            'imageSize': const ui.Size(900, 1600),
            'targetSize': const ui.Size(400, 400)
          },
          {
            'name': 'Square',
            'imageSize': const ui.Size(1000, 1000),
            'targetSize': const ui.Size(400, 600)
          },
          {
            'name': 'Ultra-wide',
            'imageSize': const ui.Size(2000, 500),
            'targetSize': const ui.Size(400, 400)
          },
          {
            'name': 'Ultra-tall',
            'imageSize': const ui.Size(500, 2000),
            'targetSize': const ui.Size(400, 400)
          },
        ];
        ...
        for (final testCase in testCases) {
          ...
          final sourceImage = await _createTestImage(
          ...
          expect(result.success, isTrue,
              reason: '$name aspect ratio should process successfully');
          expect(result.image.width, equals(targetSize.width.toInt()),
              reason: '$name output width should match target');
          expect(result.image.height, equals(targetSize.height.toInt()),
              reason: '$name output height should match target');
          expect(result.cropResult.bestCrop.isValid, isTrue,
              reason: '$name should produce valid crop coordinates');
        }
      });
      ...
      test('should handle different crop settings correctly', () async {
        ...
        final settingsVariations = [
          ...
          const CropSettings(
              enableRuleOfThirds: false, enableCenterWeighting: true),
          const CropSettings(
              enableEntropyAnalysis: true, enableEdgeDetection: false),
        ];
        ...
        for (int i = 0; i < settingsVariations.length; i++) {
          ...
          expect(result.success, isTrue,
              reason: 'Settings variation $i should process successfully');
          expect(result.cropResult.bestCrop.isValid, isTrue,
              reason: 'Settings variation $i should produce valid crop');
    ...
    group('Performance and Scalability', () {
      ...
      test('should handle memory pressure gracefully', () async {
        ...
        for (int i = 0; i < 3; i++) {
          ...
          expect(result.success, isTrue,
              reason: 'Memory pressure test $i should succeed');
          expect(result.image.width, equals(600));
          expect(result.image.height, equals(600));
        }
      });
    ...
    group('Settings Integration', () {
      test('should integrate with SmartCropPreferences correctly', () async {
        ...
        expect(conservativeSettings.aggressiveness,
            isNot(equals(aggressiveSettings.aggressiveness)));
      });
      ...
      test('should handle settings changes and cache invalidation', () async {
        ...
        final result1 = await SmartCropper.processImage(
            imageUrl, sourceImage, targetSize, initialSettings);
        expect(result1.success, isTrue);
        expect(result1.cropResult.fromCache, isFalse);

        // Process again with same settings (should use cache)
        final result2 = await SmartCropper.processImage(
            imageUrl, sourceImage, targetSize, initialSettings);
        ...
        const newSettings =
            CropSettings(aggressiveness: CropAggressiveness.aggressive);
        final result3 = await SmartCropper.processImage(
            imageUrl, sourceImage, targetSize, newSettings);
    ...
    group('Visual Regression Prevention', () {
      test('should produce consistent results for reference images', () async {
        ...
        for (int i = 0; i < referenceImages.length; i++) {
          ...
          for (int j = 0; j < 3; j++) {
            final result = await SmartCropper.processImage(
                imageUrl, image, targetSize, settings);
            expect(result.success, isTrue);
            results.add(result.cropResult);
          }
          ...
          for (int j = 1; j < results.length; j++) {
            expect(
                results[j].bestCrop.x, closeTo(results[0].bestCrop.x, 0.001));
            expect(
                results[j].bestCrop.y, closeTo(results[0].bestCrop.y, 0.001));
            expect(results[j].bestCrop.width,
                closeTo(results[0].bestCrop.width, 0.001));
            expect(results[j].bestCrop.height,
                closeTo(results[0].bestCrop.height, 0.001));
          }
        }
      });

      test('should maintain crop quality metrics within expected ranges',
          () async {
        ...
        final result = await SmartCropper.processImage(
            imageUrl, testImage, targetSize, settings);
...
Future<ui.Image> _createPatternTestImage(int width, int height) async {
  ...
  for (int y = 0; y < height; y += tileSize.toInt()) {
    for (int x = 0; x < width; x += tileSize.toInt()) {
      final isBlack =
          ((x ~/ tileSize.toInt()) + (y ~/ tileSize.toInt())) % 2 == 0;
      canvas.drawRect(
