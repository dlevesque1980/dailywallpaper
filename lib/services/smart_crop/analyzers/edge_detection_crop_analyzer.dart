...
class EdgeDetectionCropAnalyzer implements CropAnalyzer {
  ...
  Future<CropScore> analyze(ui.Image image, ui.Size targetSize) async {
    ...
    final grayscaleData =
        _convertToGrayscale(imageData, image.width, image.height);

    // Apply edge detection
    final edgeData =
        _applySobelEdgeDetection(grayscaleData, image.width, image.height);

    // Generate crop candidates based on edge density
    final candidates =
        _generateEdgeBasedCandidates(imageSize, targetAspectRatio, edgeData);
  ...
  Uint8List _applySobelEdgeDetection(
  ...
  List<CropCoordinates> _generateFallbackCandidates(
  ...
  double _calculateEdgeDensity(double centerX, double centerY,
      ui.Size imageSize, Uint8List edgeData, double radius) {
  ...
  double _scoreEdgeDistribution(
    ...
    for (int qy = 0; qy < 2; qy++) {
      for (int qx = 0; qx < 2; qx++) {
        ...
        final quadrantScore =
            _scoreAverageEdgeStrength(quadrantCrop, imageSize, edgeData);
    ...
    final distributionScore =
        mean > 0.05 ? math.max(0.0, 1.0 - variance * 10) : 0.0;
  ...
  Map<String, double> _calculateMetrics(
    ...
    return {
      'average_edge_strength':
          _scoreAverageEdgeStrength(crop, imageSize, edgeData),
