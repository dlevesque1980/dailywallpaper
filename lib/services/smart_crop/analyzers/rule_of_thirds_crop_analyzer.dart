...
class RuleOfThirdsCropAnalyzer implements CropAnalyzer {
  ...
  List<CropCoordinates> _generateCropCandidates(
    ...
    final intersectionPoints = [
      ...
      const ui.Offset(
          0.5, 0.5), // Center (not rule of thirds but good fallback)
    ];
  ...
  List<CropCoordinates> _generateEdgeAlignedCrops(
  ...
  double _scoreContentDistribution(CropCoordinates crop) {
    ...
    final edgePenalty =
        math.min(marginLeft, marginRight) + math.min(marginTop, marginBottom);

    // Prefer balanced crops
    final balance = 1.0 -
        (marginLeft - marginRight).abs() -
        (marginTop - marginBottom).abs();
