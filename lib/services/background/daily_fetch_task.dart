...
class DailyFetchTask {
  ...
  static Future<bool> execute() async {
    ...
    try {
      ...
      debugPrint('Background Task: Successfully fetched ${images.length} images.');

      // 3. Pre-download source images to local disk
      final imageCache = ImageCacheServiceImpl();
      int downloaded = 0;
      for (final image in images) {
        final path = await imageCache.downloadAndSaveSourceImage(image.url, image.imageIdent);
        if (path != null) downloaded++;
      }
      debugPrint('Background Task: Pre-downloaded $downloaded source images to local disk.');

      // 4. Clean up old local cache files (older than 2 days)
      final deleted = await imageCache.cleanupOldWallpapers(maxAge: const Duration(days: 2));
