...
import 'crop_cache_hit_rate.dart';
...
class CropCacheManager {
  ...
  Future<List<CropCacheEntry>> getCachedCropsForImage(String imageUrl) async {
    try {
      return await _dao.getByImageUrl(imageUrl);
    } catch (e) {
      return [];
    }
  }
  ...
  Future<int> invalidateImageCache(String imageUrl) async {
    try {
      return await _dao.deleteByImageUrl(imageUrl);
    } catch (e) {
      return 0;
    }
  }
  ...
  Future<int> clearCache() async {
    try {
      ...
      return await _dao.clear().timeout(
