...
class EdgeDensityPoint {
  final double x;
  final double y;
  final double density;
  EdgeDensityPoint(this.x, this.y, this.density);
}

Map<String, dynamic> performEdgeAnalysisIsolate(Map<String, dynamic> params) {
  ...
  final core = EdgeAnalyzerCore(strategyName);
  ...
  final candidates =
      core.generateEdgeBasedCandidates(imageSize, targetAspectRatio, edgeData);
...
class EdgeAnalyzerCore {
  final String strategyName;
  EdgeAnalyzerCore(this.strategyName);

  Uint8List convertToGrayscale(Uint8List rgbaData, int width, int height) {
    final grayscale = Uint8List(width * height);
    for (int i = 0; i < width * height; i++) {
      final rgbaIndex = i * 4;
      final gray = (0.299 * rgbaData[rgbaIndex] +
              0.587 * rgbaData[rgbaIndex + 1] +
              0.114 * rgbaData[rgbaIndex + 2])
          .round();
  ...
  Uint8List applySobelEdgeDetection(
    ...
    for (int y = 1; y < height - 1; y++) {
      for (int x = 1; x < width - 1; x++) {
        ...
        edgeData[y * width + x] =
            (math.sqrt(gx * gx + gy * gy) / 4).clamp(0, 255).round();
      }
    }
    return edgeData;
  }

  List<CropCoordinates> generateEdgeBasedCandidates(
      ui.Size imageSize, double targetAspectRatio, Uint8List edgeData) {
    ...
    final densities = <EdgeDensityPoint>[];
    ...
    for (int i = 0; i < gridSize; i++) {
      for (int j = 0; j < gridSize; j++) {
        ...
        final density =
            calculateEdgeDensity(centerX, centerY, imageSize, edgeData, 0.15);
        densities.add(EdgeDensityPoint(centerX, centerY, density));
      }
    }
    ...
    for (final point in densities.take(8)) {
      if (point.density > 0.1) {
        final cropX =
            math.max(0.0, math.min(1.0 - cropWidth, point.x - cropWidth / 2));
        final cropY =
            math.max(0.0, math.min(1.0 - cropHeight, point.y - cropHeight / 2));
        candidates.add(CropCoordinates(
            x: cropX,
            y: cropY,
            width: cropWidth,
            height: cropHeight,
            confidence: point.density,
  ...
  List<CropCoordinates> generateFallbackCandidates(
    ...
    for (final position in positions) {
      fallbacks.add(CropCoordinates(
          x: math.max(
              0.0, math.min(1.0 - cropWidth, position.dx - cropWidth / 2)),
          y: math.max(
  ...
  double calculateEdgeDensity(double centerX, double centerY, ui.Size imageSize,
      Uint8List edgeData, double radius) {
    ...
    int sampleCount = 0;
    for (int y = math.max(0, pixelY - pixelRadius);
        y <= math.min(height - 1, pixelY + pixelRadius);
        y++) {
      for (int x = math.max(0, pixelX - pixelRadius);
          x <= math.min(width - 1, pixelX + pixelRadius);
          x++) {
        if (math.pow(x - pixelX, 2) + math.pow(y - pixelY, 2) <=
  ...
  double scoreEdgeContent(
      CropCoordinates crop, ui.Size imageSize, Uint8List edgeData) {
    double score = 0.0;
  ...
  double scoreAverageEdgeStrength(
      CropCoordinates crop, ui.Size imageSize, Uint8List edgeData) {
    final width = imageSize.width.toInt();
    final height = imageSize.height.toInt();
    ...
    int sampleCount = 0;
    final sampleStep =
        math.max(1, math.min(cropPixelWidth, cropPixelHeight) ~/ 16);
  ...
  double scoreEdgeDistribution(
      CropCoordinates crop, ui.Size imageSize, Uint8List edgeData) {
    final quadrantScores = <double>[];
    for (int qy = 0; qy < 2; qy++) {
      for (int qx = 0; qx < 2; qx++) {
        final quadrantCrop = CropCoordinates(
            x: crop.x + (qx * crop.width / 2),
            y: crop.y + (qy * crop.height / 2),
            width: crop.width / 2,
            height: crop.height / 2,
            confidence: 0.0,
            strategy: strategyName);
        quadrantScores
            .add(scoreAverageEdgeStrength(quadrantCrop, imageSize, edgeData));
      }
    }
  ...
  double scoreStrongEdgeCount(
    ...
    int totalSamples = 0;
    final sampleStep =
        math.max(1, math.min(cropPixelWidth, cropPixelHeight) ~/ 12);
    for (int y = cropPixelY;
        y < cropPixelY + cropPixelHeight;
        y += sampleStep) {
      for (int x = cropPixelX;
  ...
  Map<String, double> calculateMetricsSync(
    ...
    return {
      'average_edge_strength':
          scoreAverageEdgeStrength(crop, imageSize, edgeData),
