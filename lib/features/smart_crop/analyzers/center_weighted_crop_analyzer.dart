...
class CenterWeightedCropAnalyzer implements CropAnalyzer {
  ...
  Future<CropScore> analyze(ui.Image image, ui.Size targetSize) async {
    ...
    final candidates =
        _generateCenterBiasedCandidates(imageSize, targetAspectRatio);
  ...
  List<CropCoordinates> _generateCenterBiasedCandidates(
  ...
  List<CropCoordinates> _generateEdgeSafeCrops(
  ...
  double _scoreCenterDistance(CropCoordinates crop) {
    ...
    final distanceFromCenter = math
        .sqrt(math.pow(cropCenterX - 0.5, 2) + math.pow(cropCenterY - 0.5, 2));
  ...
  double _scoreEdgeSafety(CropCoordinates crop) {
    ...
    final minMargin = math.min(
        math.min(leftMargin, rightMargin), math.min(topMargin, bottomMargin));
  ...
  double _scoreAspectRatioPreservation(
      CropCoordinates crop, ui.Size imageSize) {
    final imageAspectRatio = imageSize.width / imageSize.height;
    final cropAspectRatio = crop.width / crop.height;

    // Calculate how much the crop changes the aspect ratio
    final aspectRatioChange =
        (cropAspectRatio - imageAspectRatio).abs() / imageAspectRatio;
