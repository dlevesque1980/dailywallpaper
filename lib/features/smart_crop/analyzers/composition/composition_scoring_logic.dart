...
class CompositionScoringLogic {
  ...
  static double scoreComposition(
      CropCoordinates crop, ui.Size size, Uint8List data) {
    double score = 0.0;
  ...
  static Map<String, double> getDetailedScores(
      CropCoordinates crop, ui.Size size, Uint8List data) {
    return {
  ...
  static double scoreGoldenRatio(CropCoordinates crop) {
    final c = ui.Offset(crop.x + crop.width / 2, crop.y + crop.height / 2);
    final points = [
      const ui.Offset(goldenRatio, goldenRatio),
      ui.Offset(1 - goldenRatio, goldenRatio),
      ui.Offset(goldenRatio, 1 - goldenRatio),
      ui.Offset(1 - goldenRatio, 1 - goldenRatio)
    ];
    double minD = points.map((p) => (c - p).distance).reduce(math.min);
    return math.max(0.0, 1.0 - minD * 2.5);
  }

  static double scoreVisualWeight(
      CropCoordinates crop, ui.Size size, Uint8List data) {
    final w = size.width.toInt(), h = size.height.toInt();
    final cL = (crop.x * w).round(),
        cT = (crop.y * h).round(),
        cR = ((crop.x + crop.width) * w).round(),
        cB = ((crop.y + crop.height) * h).round();
    final mX = (cL + cR) ~/ 2, mY = (cT + cB) ~/ 2;
    final qs = [
      _calcWeight(data, w, h, cL, mX, cT, mY),
      _calcWeight(data, w, h, mX, cR, cT, mY),
      _calcWeight(data, w, h, cL, mX, mY, cB),
      _calcWeight(data, w, h, mX, cR, mY, cB)
    ];
    final total = qs.reduce((a, b) => a + b);
    if (total == 0) return 0.5;
    final mean = total / 4;
    final variance =
        qs.map((v) => math.pow(v / total - 0.25, 2)).reduce((a, b) => a + b) /
            4;
    return math.max(0.0, 1.0 - (variance - 0.15).abs() * 5);
  }

  static double _calcWeight(
    ...
    for (int y = sY; y < eY; y += 3) {
      for (int x = sX; x < eX; x += 3) {
        ...
        if (b >= 0) {
          final idx = (y * w + x) * 4;
          final max =
              math.max(data[idx], math.max(data[idx + 1], data[idx + 2]));
          final min =
              math.min(data[idx], math.min(data[idx + 1], data[idx + 2]));
          final s = max == 0 ? 0.0 : (max - min) / max;
          total += (b / 255.0 * 0.7 + s * 0.3);
          count++;
        }
      }
    }
  ...
  static double scoreBalance(CropCoordinates crop) {
    ...
    final dist = (ui.Offset(crop.x + crop.width / 2, crop.y + crop.height / 2) -
