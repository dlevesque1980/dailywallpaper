...
class LandscapeScoringLogic {
  static double scoreLandscapeCrop(
      CropCoordinates crop, ui.Size imageSize, Uint8List data) {
    double score = 0.0;
  ...
  static double _scoreHorizon(
    ...
    if (horizonY >= crop.y && horizonY <= crop.y + crop.height) {
      ...
      return math.max(
          0.0, 1.0 - math.min((pos - 1 / 3).abs(), (pos - 2 / 3).abs()) * 3);
    }
    return 0.6;
  }

  static double _scoreSubjects(
      CropCoordinates crop, ui.Size imageSize, Uint8List data) {
    final subjects =
        LandscapeFeatureDetector.detectSubjectAreas(imageSize, data);
  ...
  static double _scoreComplexity(
      CropCoordinates crop, ui.Size imageSize, Uint8List data) {
    double total = 0.0;
    int count = 0;
    for (double y = crop.y + 0.1; y < crop.y + crop.height - 0.1; y += 0.2) {
      for (double x = crop.x + 0.1; x < crop.x + crop.width - 0.1; x += 0.2) {
        total += LandscapeFeatureDetector.calculateLocalComplexity(
            data,
            imageSize.width.toInt(),
            imageSize.height.toInt(),
            (x * imageSize.width).round(),
            (y * imageSize.height).round(),
            15);
        count++;
      }
    }
