...
class ColorScoringLogic {
  static double scoreColorCrop(CropCoordinates crop, ui.Size size,
      Uint8List data, ImageColorAnalysis analysis) {
  ...
  static double scoreVibrancy(
      CropCoordinates crop, ui.Size size, Uint8List data) {
    final w = size.width.toInt(), h = size.height.toInt();
    double total = 0;
    int count = 0;
    for (int y = (crop.y * h).round();
        y < ((crop.y + crop.height) * h).round();
        y += 3) {
      for (int x = (crop.x * w).round();
          x < ((crop.x + crop.width) * w).round();
          x += 3) {
        if (x < w && y < h) {
          final idx = (y * w + x) * 4;
          if (idx + 2 < data.length) {
            final hsv = ColorFeatureDetector.rgbToHsv(
                data[idx], data[idx + 1], data[idx + 2]);
            total += hsv.saturation * hsv.value;
            count++;
          }
        }
      }
  ...
  static double scoreHarmony(
    ...
    for (int y = (crop.y * h).round();
      ...
      for (int x = (crop.x * w).round();
        ...
        if (x < w && y < h) {
          final idx = (y * w + x) * 4;
          if (idx + 2 < data.length)
            hues.add(ColorFeatureDetector.rgbToHsv(
                    data[idx], data[idx + 1], data[idx + 2])
                .hue);
        }
      }
    ...
    for (int i = 0; i < hues.length && comps < 50; i += 10) {
      for (int j = i + 10; j < hues.length && comps < 50; j += 10) {
        final diff = math.min(
            (hues[i] - hues[j]).abs(), 360 - (hues[i] - hues[j]).abs());
        if (diff < 30)
          score += 1.0;
  ...
  static double scoreDistribution(
      CropCoordinates crop, ui.Size size, Uint8List data) {
    final w = size.width.toInt(), h = size.height.toInt();
    final hist = <int, int>{};
    int total = 0;
