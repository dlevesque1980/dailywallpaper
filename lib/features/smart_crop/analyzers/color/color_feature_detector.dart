  ...
  static ImageColorAnalysis analyze(ui.Size size, Uint8List data) {
    ...
    for (int y = 0; y < h; y += 4) {
      for (int x = 0; x < w; x += 4) {
        ...
        totalS += hsv.saturation;
        totalV += hsv.value;
        pixels++;
        if (hsv.saturation > 0.5 && hsv.value > 0.4) {
          vibrant.add(VibrantRegion(
    ...
    final dominant = sorted
        .take(5)
        .map((e) => ui.Color.fromARGB(
            255, (e.key >> 16) & 0xFF, (e.key >> 8) & 0xFF, e.key & 0xFF))
        .toList();
    return ImageColorAnalysis(
        dominantColors: dominant,
        vibrantRegions: vibrant,
        averageSaturation: pixels > 0 ? totalS / pixels : 0,
        averageBrightness: pixels > 0 ? totalV / pixels : 0,
        colorVariety: counts.length / math.max(1, pixels ~/ 100));
  }
