...
class BirdScoringLogic {
  ...
  static double _scoreHeadQuality(CropCoordinates crop, DetectedBird bird) {
    ...
    double headPositionScore =
        headInCropY > 0.6 ? math.max(0.3, 1.0 - (headInCropY - 0.6) * 2) : 1.0;
    return bird.confidence * headPositionScore * (bird.hasBeak ? 1.2 : 1.0);
  }

  static double _scoreBirdCenterPosition(
      CropCoordinates crop, DetectedBird bird) {
    final dist = (ui.Offset((bird.center.dx - crop.x) / crop.width,
                (bird.center.dy - crop.y) / crop.height) -
  ...
  static double _scoreBirdComposition(CropCoordinates crop, DetectedBird bird) {
    ...
    for (final tx in thirdPoints) {
      for (final ty in thirdPoints) {
        bestDist = math.min(
            bestDist, (ui.Offset(birdX, birdY) - ui.Offset(tx, ty)).distance);
      }
    }
