...
class SubjectDetector {
  ...
  static List<DetectedSubject> _detectByColor(ui.Size size, Uint8List data) {
    ...
    for (int ry = 0; ry < h ~/ rSize; ry++) {
      for (int rx = 0; rx < w ~/ rSize; rx++) {
        final sX = rx * rSize,
            eX = math.min((rx + 1) * rSize, w),
            sY = ry * rSize,
            eY = math.min((ry + 1) * rSize, h);
        ...
        if (distinct > 0.3) {
          ...
          list.add(DetectedSubject(
              ...
              confidence: distinct,
              importance:
                  distinct * s * AnalyzerUtils.getPositionWeight(cX, cY)));
        }
      }
    }
    return list;
  ...
  static ui.Color _getDomColor(
    ...
    return c == 0
        ...
        : ui.Color.fromARGB(255, r ~/ c, g ~/ c, b ~/ c);
  }
  ...
  static double _calcDistinct(ui.Color col, Uint8List data, int w, int h) {
    ...
    for (int y = 0; y < h; y += 8) {
      for (int x = 0; x < w; x += 8) {
        final idx = (y * w + x) * 4;
        if (idx + 2 < data.length) {
          final d = math.sqrt(math.pow(data[idx] - (col.r * 255).round(), 2) +
              math.pow(data[idx + 1] - (col.g * 255).round(), 2) +
              math.pow(data[idx + 2] - (col.b * 255).round(), 2));
          if (d < 50) sim++;
          tot++;
        }
      }
    ...
    for (final scale in [16, 24, 32, 48]) {
      ...
      for (int y = scale; y < h - scale; y += step) {
        for (int x = scale; x < w - scale; x += step) {
          ...
          if (circ > 0.3) {
            list.add(DetectedSubject(
                center: ui.Offset(x / w, y / h),
                bounds: ui.Rect.fromLTWH((x - scale) / w, (y - scale) / h,
                    (scale * 2) / w, (scale * 2) / h),
                type: SubjectType.circularShape,
                confidence: circ,
                importance: circ *
                    (scale * 2 / math.min(w, h)) *
                    AnalyzerUtils.getPositionWeight(x / w, y / h) *
                    1.2));
          }
        }
      }
  ...
  static double _calcCirc(Uint8List data, int w, int cx, int cy, int r) {
    ...
    for (int a = 0; a < 360; a += 15) {
      ...
      final eb = AnalyzerUtils.getPixelBrightness(data, w,
          cx + (r * math.cos(rad)).round(), cy + (r * math.sin(rad)).round());
      if (eb >= 0) edges.add(eb);
    }
    if (edges.isEmpty) return 0.0;
    final mean = edges.reduce((a, b) => a + b) / edges.length;
    final varB =
        edges.map((s) => math.pow(s - mean, 2)).reduce((a, b) => a + b) /
            edges.length;
    return (centerB - mean).abs() /
        255.0 *
        (1.0 - math.min(1.0, varB / (255 * 255)));
  }
  ...
  static List<DetectedSubject> _merge(List<DetectedSubject> list) {
    ...
    for (int i = 0; i < list.length; i++) {
      ...
      final group = <DetectedSubject>[list[i]];
      processed[i] = true;
      for (int j = i + 1; j < list.length; j++) {
        if (!processed[j] &&
            (list[i].center - list[j].center).distance < 0.15) {
          group.add(list[j]);
          processed[j] = true;
        }
      }
